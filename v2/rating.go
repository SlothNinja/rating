...
import (
	...
	"github.com/SlothNinja/contest/v2"
...
func New(c *gin.Context, id int64, pk *datastore.Key, t game.Type, params ...float64) *Rating {
...
func NewCurrent(pk *datastore.Key, t game.Type, params ...float64) *CurrentRating {
	r, rd := defaultR, defaultRD
	if len(params) == 2 {
		r, rd = params[0], params[1]
	}
...
// Get Current Rating for game.Type and user associated with uKey
func (client *Client) Get(c *gin.Context, uKey *datastore.Key, t game.Type) (*CurrentRating, error) {
	ratings, err := client.GetMulti(c, []*datastore.Key{uKey}, t)
	return ratings[0], singleError(err)
}

func (client *Client) GetMulti(c *gin.Context, uKeys []*datastore.Key, t game.Type) (CurrentRatings, error) {
	l := len(uKeys)
...
func (client *Client) GetFor(c *gin.Context, t game.Type) (CurrentRatings, error) {
...
func (client *Client) getFiltered(c *gin.Context, t game.Type, leader bool, offset, limit int32) (CurrentRatings, int64, error) {
	q := getAllQuery(c)
...
func (client *Client) projected(c *gin.Context, ukey *datastore.Key, t game.Type, cs ...*contest.Contest) (*CurrentRating, error) {
	client.Log.Debugf(msgEnter)
	defer client.Log.Debugf(msgExit)
...
func (client *Client) GetProjected(c *gin.Context, ukey *datastore.Key, t game.Type) (*CurrentRating, error) {
	client.Log.Debugf(msgEnter)
	defer client.Log.Debugf(msgExit)
...
func (client *Client) GetProjectedWith(c *gin.Context, ukey *datastore.Key, t game.Type, cs []*contest.Contest) (*CurrentRating, error) {
	client.Log.Debugf(msgEnter)
	defer client.Log.Debugf(msgExit)
...
func (client *Client) For(c *gin.Context, u *user.User, t game.Type) (*CurrentRating, error) {
...
func (client *Client) JSONFilteredAction(c *gin.Context) {
	...
	t := game.ToType(strings.ToLower(c.Param("type")))
...
func (client *Client) IncreaseFor(c *gin.Context, u *user.User, t game.Type, cs []*contest.Contest) (*CurrentRating, *CurrentRating, error) {
	client.Log.Debugf(msgEnter)
	defer client.Log.Debugf(msgExit)
...
func createTask(projectID, locationID, queueID string, uid int64, t game.Type) (*taskspb.Task, error) {
		...
		Task: &taskspb.Task{
			...
			MessageType: &taskspb.Task_AppEngineHttpRequest{
				AppEngineHttpRequest: &taskspb.AppEngineHttpRequest{
					...
					RelativeUri: fmt.Sprintf("/ratings/userUpdate/%d/%s", uid, strings.ToLower(t.SString())),
				},
			},
		},
	}
